...
const API_KEY = "HGFBQ3MXcDhnu7CYAuttdngPxWsW6BNxCHnq-aK4"; // Pastikan tidak ada tanda kutip yang salah atau karakter aneh
...
const proxyListURL = 'https://raw.githubusercontent.com/gopaybis/Proxylist/refs/heads/main/proxyiplengkap5.txt';
const namaWeb = 'KUNCUNG NETWORK'
const linkTele = 'https://t.me/kopi_kapal'
const wildcards = [
  'ava.game.naver.com',
  'quiz.int.vidio.com',
  'support.zoom.us',
...
export default {
  ...
  if (proxyListMatch) {
    ...
    proxyIP = `${randomProxy.proxyIP}:${randomProxy.proxyPort}`;
    console.log(`Selected Proxy (/PROXYLIST): ${proxyIP}`);
  ...
  if (allMatch) {
    ...
    const randomProxy = selectedProxies[Math.floor(Math.random() * selectedProxies.length)];
    proxyIP = `${randomProxy.proxyIP}:${randomProxy.proxyPort}`;
    console.log(`Selected Proxy (/ALL): ${proxyIP}`);
    return await websockerHandler(request, proxyIP);
  }
  ...
  if (putarMatch) {
    ...
    const randomProxy = selectedProxies[Math.floor(Math.random() * selectedProxies.length)];
    proxyIP = `${randomProxy.proxyIP}:${randomProxy.proxyPort}`;
    console.log(`Selected Proxy (/PUTAR${count}): ${proxyIP}`);
    return await websockerHandler(request, proxyIP);
  }
    ...
    return await websockerHandler(request, proxyIP);
      ...
      switch (url.pathname) {
        ...
        case "/sub":
          ...
          break;
        default:
          const targetReverseProxy = "mass-proxychecker.vercel.app";
